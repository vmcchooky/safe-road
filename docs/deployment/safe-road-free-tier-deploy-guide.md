# Deploy Safe Road for `quorix.io.vn` using free / trial infrastructure
...
> The core rule is simple: **do not touch the existing `quorix.io.vn` and `www.quorix.io.vn` records**. Deploy Safe Road on a separate subdomain.

---

## 0. Executive decision

For Safe Road, there are two very different deployment modes:

| Mode | Best platform type | Supports Docker Compose? | Supports custom domain? | Supports DoH `/dns-query`? | Supports DoT `:853`? | Suitable for full Safe Road? |
|---|---:|---:|---:|---:|---:|---:|
...
1. **If you need full Safe Road including DoT:** use a real VPS / VM trial.
...
| Platform | Free/trial type | Compute limit relevant to Safe Road | Disk/storage | Public IPv4 | Custom domain | 24/7? | Safe Road verdict |
|---|---|---:|---:|---:|---:|---:|---|
...
| AWS Free Tier | 12-month free EC2 micro | `t2.micro`/`t3.micro` class; usually 1 GB RAM, below target. | 30 GB EBS free-tier baseline. | 750 hours/month in-use public IPv4 included in EC2 Free Tier; overage billed. | Yes | Yes within 750 hours/month | Not enough RAM for full Safe Road; usable for minimal API-only test. |
...
| Koyeb Free Instance | One free web service instance | 0.1 vCPU, 512 MB RAM, 2 GB SSD. | 2 GB SSD | No direct VM IPv4 control. | Yes | Limited | Too small for full Safe Road; possible tiny API mock/demo. |
| Railway Free/Trial | Credit-based app hosting | Up to 1 vCPU / 0.5 GB RAM per service on free-style limits; credit can run out. | 0.5 GB volume storage in free plan info. | No direct VM IPv4 control. | Yes | Not reliable when credits run out | Good for quick web/API demo, not full Safe Road. |
...
| Vercel | Frontend/serverless | Great for Hugo/Next/static sites, not long-running Docker Compose. | Not VPS disk | No | Yes | Yes for frontend | Keep current Hugo site here; do not deploy Safe Road full stack here. |
...
### Profile A — Full Safe Road test on trial VM
...
This requires a **reduced Safe Road deployment**, usually one container exposing only HTTP.
...
Use one of these for Safe Road:
...
Google Cloud has an always-free `e2-micro`, but it is too weak for full Safe Road. The useful path is:
...
This is likely too small for full Docker Compose Safe Road.
...
For realistic Safe Road testing, create:
...
AWS Free Tier is useful for learning EC2, but its free VM class is usually too small for Safe Road full stack.
...
Avoid using it as the first full Safe Road deployment unless you optimize memory aggressively.
...
- set `SAFE_ROAD_AI_PROVIDER=none`.

### 8.2 Suggested environment for API-only demo

```env
SAFE_ROAD_PUBLIC_HOST=safe-api.quorix.io.vn
SAFE_ROAD_ADMIN_PASSWORD=change-me-long-random
SAFE_ROAD_ADMIN_API_KEY=change-me-long-random

SAFE_ROAD_AI_PROVIDER=none
SAFE_ROAD_MODE=demo
SAFE_ROAD_ENABLE_DOT=false
SAFE_ROAD_ENABLE_DOH=true
SAFE_ROAD_REDIS_URL=
PORT=8080
```
...
```powershell
git clone https://github.com/vmcchooky/safe-road.git
cd safe-road
...
```env
SAFE_ROAD_PUBLIC_HOST=safe-local.quorix.io.vn
SAFE_ROAD_CADDY_EMAIL=your-email@example.com

SAFE_ROAD_ADMIN_PASSWORD=your-long-random-password
SAFE_ROAD_ADMIN_API_KEY=your-long-random-api-key
...
cloudflared tunnel create safe-road-local
cloudflared tunnel route dns safe-road-local safe-local.quorix.io.vn
```

Create a config file:

```yaml
tunnel: safe-road-local
...
```powershell
cloudflared tunnel run safe-road-local
```
...
```powershell
ssh-keygen -t ed25519 -C "safe-road-vps"
```
...
```env
SAFE_ROAD_PUBLIC_HOST=safe.quorix.io.vn
SAFE_ROAD_CADDY_EMAIL=your-real-email@example.com
...
SAFE_ROAD_DUCKDNS_DOMAIN=
SAFE_ROAD_DUCKDNS_TOKEN=
```

### 13.8 Deploy

```sh
chmod +x scripts/safe-road.sh scripts/duckdns-update.sh scripts/bench.sh
...
```text
username: admin
password: SAFE_ROAD_ADMIN_PASSWORD
```
...
SAFE_ROAD_AI_PROVIDER=none
SAFE_ROAD_ENABLE_DOT=false
SAFE_ROAD_FEED_SYNC_ON_START=false
SAFE_ROAD_MAX_WORKERS=1
SAFE_ROAD_LOG_LEVEL=info
```
...
## 17. Recommended final decision for Safe Road
...
To make Safe Road deploy well on free platforms, add these project-level profiles:
...
```env
SAFE_ROAD_PUBLIC_HOST=safe-api.quorix.io.vn
SAFE_ROAD_ADMIN_PASSWORD=change-me
SAFE_ROAD_ADMIN_API_KEY=change-me
...
This makes Safe Road portable across PaaS platforms that expect one HTTP service.
