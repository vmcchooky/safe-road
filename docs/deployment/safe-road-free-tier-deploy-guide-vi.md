# Hướng dẫn deploy Safe Road bằng gói free / trial cho `quorix.io.vn`
...
Nếu mục tiêu của bạn là deploy Safe Road **đầy đủ tính năng**, gồm:
...
Nếu chỉ muốn demo nhẹ API/web thì có thể dùng Render, Koyeb, Railway hoặc Cloud Run-style hosting, nhưng đó chỉ là **bản rút gọn**, không phải Safe Road đầy đủ.
...
|---|---|---:|---:|---:|---:|---|
| **A. Full VPS / Cloud VM test** | Muốn test gần production nhất | Có | Có | Có | Có, nếu firewall mở | Nên chọn nếu muốn test Safe Road đầy đủ |
...
| Nền tảng | Loại miễn phí | Tài nguyên liên quan | Disk | Public IPv4 | Custom domain | Chạy 24/7? | Phù hợp Safe Road không? |
|---|---|---:|---:|---:|---:|---:|---|
...
| **Railway** | Trial/credit-based | App hosting, không phải VPS | Volume nhỏ | Không có VM IPv4 trực tiếp | Có | Phụ thuộc credit | Demo nhanh, không full Safe Road |
...
| **Vercel** | Frontend/serverless | Rất tốt cho Hugo/Next/static | Không phải VPS disk | Không | Có | Có cho frontend | Giữ Hugo site ở đây, không deploy Safe Road full stack |
...
Tạo subdomain riêng cho Safe Road:
...
```text
SAFE_ROAD_ADMIN_PASSWORD = mật khẩu dài, lưu trong password manager
SAFE_ROAD_ADMIN_API_KEY  = chuỗi hex 64 ký tự
```
...
SAFE_ROAD_CADDY_EMAIL=email-thật-của-bạn@example.com
```
...
Vì sao để `SAFE_ROAD_AI_PROVIDER=none` trước?
...
Đây là hướng nên dùng nếu bạn muốn test Safe Road giống production nhất.
...
Google Cloud always-free `e2-micro` quá yếu cho Safe Road đầy đủ, nhưng nếu tài khoản mới có trial credit thì bạn có thể tạo VM tốt hơn để test ngắn hạn.
...
AWS Free Tier thường chỉ cho EC2 micro 1 GB RAM. Safe Road full stack có thể chạy rất căng, dễ thiếu RAM.
...
Chỉ thêm record cho subdomain Safe Road.
...
```sh
scripts/safe-road.sh backup
```

Restore:

```sh
scripts/safe-road.sh restore /full/path/to/redis-dump.rdb
```
...
1. Đăng nhập Render.
2. New → Web Service.
3. Kết nối GitHub repo `safe-road`.
4. Chọn branch.
...
Bạn chạy Safe Road trên laptop/PC ở nhà, rồi dùng Cloudflare Tunnel để public HTTPS.
...
| Nhu cầu | Nên chọn |
|---|---|
| Test đầy đủ Safe Road, có DoT `853` | Azure Free Account / Google Cloud trial VM / VPS rẻ |
...
SAFE_ROAD_ADMIN_PASSWORD=replace-with-long-password
SAFE_ROAD_ADMIN_API_KEY=replace-with-long-api-key
