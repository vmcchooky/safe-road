# Gemini Handoff - Safe Road on `quorix.io.vn`

Date: 2026-05-25

You are helping me finish the initial VPS deployment for the Safe Road project.
...
- I have not created any Safe Road subdomain yet.
- The deployment must not break the existing root site.

## Hard requirements

- Keep `quorix.io.vn` and `www.quorix.io.vn` on Vercel.
- Use a separate subdomain for Safe Road, preferably `safe.quorix.io.vn`.
...
5. Exactly what DNS record to add in PA Viet Nam for the Safe Road subdomain.
...
- The Safe Road service should live on a subdomain only.
...
- `docs/deployment/safe-road-free-tier-deploy-guide-vi.md`
...
- `ops/cron/safe-road.cron.example`
- `scripts/safe-road.ps1`
- `scripts/safe-road.sh`
