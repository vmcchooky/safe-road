# Deploy Safe Road for `quorix.io.vn` step by step
...
- Goal: keep the existing Vercel site working, and deploy Safe Road on a separate subdomain
...
Use a separate subdomain for Safe Road:

- `safe.quorix.io.vn` -> Safe Road public HTTPS endpoint

This is the safest path because it avoids breaking the current website.

Safe Road public endpoints would then be:
...
- Use the hex output for `SAFE_ROAD_ADMIN_API_KEY`
- Use a long password-manager-generated password for `SAFE_ROAD_ADMIN_PASSWORD`
...
What you should add is only the Safe Road subdomain:
...
Why `SAFE_ROAD_AI_PROVIDER=none` first:
...
- [safe-road-production.cron.example](D:/Go/duan/safe-road/ops/cron/safe-road-production.cron.example:1)
...
- `SAFE_ROAD_DUCKDNS_DOMAIN`: only the subname, for example `yourname`
- `SAFE_ROAD_DUCKDNS_TOKEN`: your account token from DuckDNS
...
- you do not want to attach Safe Road to `quorix.io.vn`
