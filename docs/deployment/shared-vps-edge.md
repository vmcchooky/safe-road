# Safe Road On The Shared Quorix VPS

Use this path when Safe Road runs behind the host-level Caddy edge for all Quorix apps.
...
- `127.0.0.1:8080` for Safe Road core API/dashboard
...
SAFE_ROAD_DNS_DOT_PUBLISHED_PORT=853
SAFE_ROAD_DNS_DOT_CONTAINER_PORT=8533
SAFE_ROAD_DNS_DOT_CERTS_DIR=./ops/certs/dot
```
...
- Do not start the internal Safe Road `caddy` service on the shared VPS.
- Keep `SAFE_ROAD_BLOCK_PAGE_IP` set to the VPS public IPv4 in production.
- Keep `SAFE_ROAD_AI_PROVIDER=none` for the first deploy; enable Gemini only after the base stack is stable.
