# Safe Road Production Completion Checklist

Date: 2026-05-21

This file is the single operational checklist for moving Safe Road from the current repository state to production-ready, and then to "perfect production". It intentionally does not merge detailed design/spec documents. Those remain in `docs/specs/` as technical references.
...
- `[x]` OpenAPI file exists at `docs/openapi/safe-road.yaml`.
...
- `[x]` In production mode, fail startup if `SAFE_ROAD_ADMIN_PASSWORD` is missing or weak.
- `[x]` In production mode, fail startup if `SAFE_ROAD_ADMIN_API_KEY` is missing.
...
1. Define `SAFE_ROAD_ENV=production` behavior.
...
- `[x]` Resolver can return `SAFE_ROAD_BLOCK_PAGE_IP` for blocked A/AAAA answers.
...
- `[x]` Add log retention policy for Docker JSON logs, not only files under `/opt/safe-road/logs/*.log`.
...
Goal: document what Safe Road protects, what it does not protect, and how it can be attacked.
