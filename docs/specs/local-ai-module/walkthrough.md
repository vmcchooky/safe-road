...
- **[NEW] [provider.go](file:///d:/Go/duan/safe-road/internal/ai/provider.go):** Định nghĩa interface `Provider` để chuẩn hóa các client AI cục bộ và đám mây.
- **[NEW] [ollama.go](file:///d:/Go/duan/safe-road/internal/ai/ollama.go):** Client ngoại tuyến tương tác trực tiếp với Ollama HTTP API, sử dụng cấu hình xuất định dạng JSON (`format: json`) đảm bảo tính toàn vẹn dữ liệu.
- **[MODIFY] [client.go](file:///d:/Go/duan/safe-road/internal/ai/client.go):** Chuyển đổi logic Gemini sang `GeminiClient` và biến `Client` thành một trình điều phối AI trung tâm. Đảm bảo tương thích ngược hoàn hảo thông qua giữ nguyên hàm khởi tạo cũ.
- **[MODIFY] [service.go](file:///d:/Go/duan/safe-road/internal/risk/service.go) & [env.go](file:///d:/Go/duan/safe-road/internal/risk/env.go):** Ánh xạ cấu hình Ollama từ `.env`, tích hợp AI client manager mới vào luồng thẩm định.
- **[MODIFY] [.env.example](file:///d:/Go/duan/safe-road/.env.example):** Bổ sung hướng dẫn chi tiết các thông số cấu hình và lưu ý về phần cứng.
...
2. Cấu hình file `.env`:
   ```env
   SAFE_ROAD_AI_PROVIDER=gemini
   SAFE_ROAD_GEMINI_API_KEY=your_free_gemini_api_key_here
...
3. Cấu hình file `.env`:
   ```env
   SAFE_ROAD_AI_PROVIDER=ollama
   SAFE_ROAD_OLLAMA_BASE_URL=http://localhost:11434
   SAFE_ROAD_OLLAMA_MODEL=gemma2:2b
   SAFE_ROAD_OLLAMA_TIMEOUT_MS=5000
   ```
