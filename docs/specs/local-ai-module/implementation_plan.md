...
> 2. **`gemini` (Mặc định):** Sử dụng Gemini API (yêu cầu `SAFE_ROAD_GEMINI_API_KEY`). **Hoàn hảo cho VPS giá rẻ** vì chỉ gọi HTTP API từ xa, không tiêu tốn thêm RAM máy chủ.
...
#### [NEW] [ollama.go](file:///d:/Go/duan/safe-road/internal/ai/ollama.go)
...
#### [NEW] [provider.go](file:///d:/Go/duan/safe-road/internal/ai/provider.go)
...
#### [MODIFY] [client.go](file:///d:/Go/duan/safe-road/internal/ai/client.go)
...
#### [MODIFY] [service.go](file:///d:/Go/duan/safe-road/internal/risk/service.go)
...
#### [MODIFY] [env.go](file:///d:/Go/duan/safe-road/internal/risk/env.go)
-   Ánh xạ các biến môi trường mới từ `.env` vào cấu trúc khởi tạo `NewServiceFromEnv()`:
    -   `SAFE_ROAD_AI_PROVIDER` (mặc định: `gemini`)
    -   `SAFE_ROAD_OLLAMA_BASE_URL` (mặc định: `http://localhost:11434`)
    -   `SAFE_ROAD_OLLAMA_MODEL` (mặc định: `gemma2:2b`)
    -   `SAFE_ROAD_OLLAMA_TIMEOUT_MS` (mặc định: `5000`)
