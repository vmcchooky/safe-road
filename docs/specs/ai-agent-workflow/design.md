...
|---------|---------|-------------|
| `SAFE_ROAD_AGENT_AUDIT_INTERVAL_SECONDS` | `3600` (1h) | Chu kỳ chạy audit |
| `SAFE_ROAD_AGENT_AUDIT_TIMEOUT_SECONDS` | `300` (5m) | Timeout mỗi lần chạy |
| `SAFE_ROAD_AGENT_AUDIT_MIN_OCCURRENCES` | `3` | Số lần xuất hiện tối thiểu |
| `SAFE_ROAD_AGENT_AUDIT_MAX_PER_CYCLE` | `50` | Giới hạn domain/chu kỳ |
| `SAFE_ROAD_AGENT_AUDIT_CONFIDENCE_THRESHOLD` | `0.7` | Ngưỡng confidence auto-block |
...
```
1. Parse SAFE_ROAD_AGENT_FEED_SOURCES → []string URLs.
...
| Env Var | Default | Description |
|---------|---------|-------------|
| `SAFE_ROAD_AGENT_FEED_SOURCES` | `""` | Comma-separated feed URLs |
| `SAFE_ROAD_AGENT_FEED_INTERVAL_SECONDS` | `86400` (24h) | Chu kỳ sync |
| `SAFE_ROAD_AGENT_FEED_TIMEOUT_SECONDS` | `120` (2m) | Timeout mỗi lần sync |
...
3. Build payload:
   {
     "timestamp": "...",
     "event_type": "safe_road_agent_alert",
     "summary": "Safe Road: 3 domains auto-blocked, 1 feed sync error",
     "events": [
...
| Env Var | Default | Description |
|---------|---------|-------------|
| `SAFE_ROAD_AGENT_WEBHOOK_URL` | `""` | Webhook URL (empty = disabled) |
| `SAFE_ROAD_AGENT_ALERT_INTERVAL_SECONDS` | `900` (15m) | Chu kỳ check & gửi alert |
| `SAFE_ROAD_AGENT_ALERT_TIMEOUT_SECONDS` | `30` | Timeout gửi webhook |
| `SAFE_ROAD_AGENT_ALERT_MIN_EVENTS` | `1` | Số events tối thiểu để trigger |
