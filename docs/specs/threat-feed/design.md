...
- `safe-road:threat:feed`: Redis Set of normalized domains.
- Existing analysis cache remains `safe-road:analysis:<domain>`.
...
- `-source`: local file path or HTTP/HTTPS URL. Defaults to `SAFE_ROAD_THREAT_FEED_SOURCE`.
- `-redis-addr`: defaults to `SAFE_ROAD_REDIS_ADDR`.
- `-redis-password`: defaults to `SAFE_ROAD_REDIS_PASSWORD`.
- `-redis-db`: defaults to `SAFE_ROAD_REDIS_DB`.
- `-key`: defaults to `SAFE_ROAD_THREAT_FEED_KEY`, then `safe-road:threat:feed`.
...
3. Check exact match in `safe-road:threat:feed`.
