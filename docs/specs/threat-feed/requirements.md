...
Add a local, zero-cost threat feed path so Safe Road can block known bad domains from a configurable file or URL before relying on lexical heuristics.

## Functional Requirements

- Feed data must be loaded into Redis Set `safe-road:threat:feed`.
...
- `cmd/feed-sync` can write valid normalized domains into `safe-road:threat:feed`.
