...
Tài liệu này tổng hợp kết quả thiết kế, phát triển thực tế, kiểm thử tự động của hệ thống Cảnh báo nâng cao đa kênh (Telegram / Slack / Email) nhằm phát hiện và cảnh báo trực quan các hành vi giả mạo ngân hàng hoặc cơ quan nhà nước Việt Nam cho dự án Safe Road.
...
*   **[.env.example](file:///d:/Go/duan/safe-road/.env.example)** & **.env**:
...
*   **[internal/agent/alert.go](file:///d:/Go/duan/safe-road/internal/agent/alert.go)**:
...
*   **[cmd/core-api/main.go](file:///d:/Go/duan/safe-road/cmd/core-api/main.go)**:
    *   Đọc và chuyển đổi cấu hình từ môi trường (`SAFE_ROAD_ALERT_TELEGRAM_ENABLED`, v.v.).
    *   Đăng ký `AlertTask` vào `agentEngine` với chu kỳ quét mặc định 15 phút.

### 1.4. Bộ kiểm thử tự động
*   **[internal/agent/alert_test.go](file:///d:/Go/duan/safe-road/internal/agent/alert_test.go)**:
...
...
PASS
ok  	safe-road/internal/agent	13.265s
```
