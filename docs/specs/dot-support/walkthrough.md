...
2.  **Cơ chế TLS an toàn cho local và production:**
    ...
    *   Nếu người vận hành đã cấu hình `SAFE_ROAD_DNS_DOT_CERT_FILE` hoặc `SAFE_ROAD_DNS_DOT_KEY_FILE` nhưng file lỗi/thiếu/không đọc được, `dns-resolver` ghi log lỗi và thoát ngay để tránh production chạy với chứng chỉ không được client tin cậy.
