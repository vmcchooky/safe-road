...
```go
// Cache key format:
// "safe-road:tls:{domain}" → JSON of tlsinspect.Result
// "safe-road:whois:{registeredDomain}" → JSON of whois.Result

// Before network call:
cached, err := s.redis.Get(ctx, "safe-road:tls:"+domain)
if err == nil {
    return unmarshalTLSResult(cached)
}

// After network call:
s.redis.Set(ctx, "safe-road:tls:"+domain, marshalTLSResult(result), ttl)
```
...
|---|---|---|
| `SAFE_ROAD_ENRICH_ENABLED` | `true` | Enable/disable TLS+WHOIS enrichment |
| `SAFE_ROAD_ENRICH_TIMEOUT_MS` | `3000` | Combined timeout for TLS+WHOIS (ms) |
| `SAFE_ROAD_TLS_INSPECT_ENABLED` | `true` | Enable/disable TLS inspection |
| `SAFE_ROAD_WHOIS_ENABLED` | `true` | Enable/disable WHOIS lookup |
