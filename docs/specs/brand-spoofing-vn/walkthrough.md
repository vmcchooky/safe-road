# Walkthrough: Safe Road Enhancements

Hệ thống Safe Road hiện đã hoàn thành hai cấu phần cốt lõi nâng cao:
...
Safe Road giờ đây có khả năng **lưu trữ dữ liệu lâu dài** (persistent) và **quản lý domain bằng API** nhờ tích hợp SQLite, bên cạnh Redis (vẫn giữ vai trò cache + threat feed).
...
Hệ thống điều phối tác vụ tự động (Agent Engine) giúp Safe Road vận hành độc lập, tự động phân tích rủi ro chuyên sâu và tương tác với các hệ thống bên ngoài.
...
Để ngăn ngừa việc chặn nhầm các tên miền phổ biến và uy tín (False Positives), Safe Road đã tích hợp danh sách **Whitelist Tranco Top 1M** với cơ chế tối ưu hóa bộ nhớ RAM đột phá cùng khả năng tự động cập nhật không gián đoạn dịch vụ.

### Kiến trúc Hybrid Bloom-SQLite (RAM & Disk Co-working)
Thay vì nạp toàn bộ 1.000.000 tên miền vào RAM thông qua một cấu trúc map truyền thống `map[string]struct{}` (tiêu tốn hơn **100MB RAM**), Safe Road sử dụng giải pháp lai:
...
```env
# --- SQLite Telemetry ---
SAFE_ROAD_SQLITE_PATH=./data/safe-road.db
SAFE_ROAD_TELEMETRY_RETENTION_DAYS=30

# --- AI Agent Workflow ---
SAFE_ROAD_AGENT_ENABLED=true                # Bật/tắt toàn bộ hệ thống Agent
SAFE_ROAD_AGENT_AUDIT_INTERVAL=5m           # Tần suất chạy tác vụ tuần tra làm giàu thông tin
SAFE_ROAD_AGENT_AUDIT_TIMEOUT=1m
SAFE_ROAD_AGENT_AUDIT_LIMIT_PER_CYCLE=10    # Số lượng domain tối đa phân tích mỗi chu kỳ

SAFE_ROAD_AGENT_FEED_SYNC_INTERVAL=1h       # Tần suất đồng bộ threat feeds
SAFE_ROAD_AGENT_FEED_SYNC_TIMEOUT=5m
SAFE_ROAD_AGENT_FEED_SOURCES=https://raw.githubusercontent.com/StevenBlack/hosts/master/hosts

SAFE_ROAD_AGENT_ALERT_INTERVAL=1m           # Tần suất quét và gửi cảnh báo
SAFE_ROAD_AGENT_ALERT_TIMEOUT=30s
SAFE_ROAD_AGENT_WEBHOOK_URL=https://discord.com/api/webhooks/...  # Địa chỉ nhận thông báo

# --- Whitelist Auto-Update & Optimization ---
SAFE_ROAD_WHITELIST_ENABLED=true                # Bật/tắt tính năng lọc nhanh qua Whitelist
SAFE_ROAD_WHITELIST_UPDATE_INTERVAL=24h         # Tần suất tự động tải và cập nhật Whitelist từ Tranco
SAFE_ROAD_WHITELIST_UPDATE_TIMEOUT=10m          # Timeout tối đa cho tác vụ cập nhật Whitelist
SAFE_ROAD_WHITELIST_SOURCE_URL=https://tranco-list.eu/top-1m.csv.zip # Nguồn danh sách Whitelist
```
...
```
ok  	safe-road/cmd/core-api	13.176s
ok  	safe-road/cmd/dns-resolver	15.319s
ok  	safe-road/cmd/feed-sync	4.116s
ok  	safe-road/internal/agent	10.563s
ok  	safe-road/internal/ai	5.159s
ok  	safe-road/internal/feed	8.685s
ok  	safe-road/internal/ratelimit	3.763s
ok  	safe-road/internal/risk	11.255s
ok  	safe-road/internal/store	7.746s
ok  	safe-road/internal/tlsinspect	4.512s
ok  	safe-road/internal/whois	2.237s
```
...
Safe Road giờ đây hỗ trợ phân cấp chính sách bảo mật chi tiết, cho phép các nhóm Client khác nhau (như Trẻ em, Nhân viên kỹ thuật, IoT) nhận được các bộ lọc và luật chặn/cho phép khác nhau một cách linh hoạt.
...
Để bảo mật an toàn cho control plane của Safe Road, hệ thống hiện đã tích hợp cơ chế xác thực kép cực kỳ an toàn, hiệu năng cao (Zero-Cost stateless session) bảo vệ cho cả Dashboard quản trị và các REST API administrative.
...
3. **Cơ chế Bảo mật mặc định (Secure by Default)**:
   - Khi khởi động, Core API đọc hai cấu hình bảo mật `SAFE_ROAD_ADMIN_PASSWORD` và `SAFE_ROAD_ADMIN_API_KEY` từ biến môi trường hoặc `.env`.
...
4. **Giao diện Đăng nhập Glassmorphic cao cấp (`login.html`)**:
   - Trang đăng nhập mang đậm phong cách Safe Road với hiệu ứng làm mờ nền kính (Backdrop filter blur), độ phát sáng viền nhẹ tinh tế, Outfit typography hiện đại và các micro-animations chuyển tiếp mượt mà.
...
Safe Road giờ đây có khả năng tự động nhận diện các hành vi lừa đảo giả mạo thương hiệu uy tín (Typosquatting/Brand Spoofing) và các tên miền sinh tự động bằng máy (DGA) thông qua các thuật toán phân tích lexical chuyên sâu ngay trên RAM với chi phí tài nguyên cực thấp (Zero-Cost).
...
* Vượt qua 100% các bài kiểm thử tự động, tích hợp hoàn hảo vào pipeline phân tích rủi ro `Analyze` giúp nâng cao vượt bậc khả năng phòng thủ của Safe Road trước các cuộc tấn công Phishing tinh vi.

---

## 11. Bảo vệ Thương hiệu & Cơ quan Việt Nam chống tội phạm công nghệ cao

Để phục vụ nhân dân Việt Nam phòng chống tội phạm công nghệ cao, đặc biệt trước các thủ đoạn lừa đảo tinh vi giả mạo website của cơ quan Chính phủ, cơ quan hành chính Nhà nước Việt Nam, và các tổ chức tài chính/ngân hàng tại Việt Nam, Safe Road đã mở rộng hệ thống nhận diện bảo vệ thương hiệu quốc gia và triển khai bộ từ khóa tiếng Việt chuyên biệt.
