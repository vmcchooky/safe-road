...
### 1.1 Mục đích
Tài liệu này là phiên bản **Zero-Cost** của SRS Safe Road, được thiết kế đặc biệt cho cộng đồng, cá nhân và tổ chức phi lợi nhuận **không có ngân sách vận hành**. Mọi quyết định kỹ thuật đều ưu tiên "free-as-in-beer" và "self-hosted".
...
| **Domain** | DuckDNS (`saferoad.duckdns.org`) hoặc No-IP | **$0** | Không mua domain trả phí. |
...
│  └─────────────────────────────────────────────────────┘  │
│                                                             │
│  Public IP (Free) ──► DuckDNS ──► saferoad.duckdns.org     │
...
curl -k "https://www.duckdns.org/update?domains=saferoad&token=YOUR_TOKEN&ip=" | cron
```
...
```bash
# Cron hàng ngày 2AM
0 2 * * * redis-cli SAVE && rclone copy /var/lib/redis/dump.rdb gdrive:safe-road-backup/$(date +\%Y\%m\%d)
```
...
| `SAFE_ROAD_DNS_DOH_PORT` | `443` | |
| `SAFE_ROAD_DNS_DOT_PORT` | `853` | |
| `SAFE_ROAD_REDIS_ADDR` | `redis:6379` | |
| `SAFE_ROAD_RISK_THRESHOLD` | `70` | |
| `SAFE_ROAD_TTL_BLOCKED` | `21600` | 6 giờ |
| `SAFE_ROAD_TTL_ALLOWED` | `10800` | 3 giờ |
| `SAFE_ROAD_WORKER_POOL_SIZE` | `8` | |
| `SAFE_ROAD_OLLAMA_HOST` | `http://ollama:11434` | |
| `SAFE_ROAD_OLLAMA_MODEL` | `gemma:2b` | |
| `SAFE_ROAD_OLLAMA_TIMEOUT` | `3000` | ms |
| `SAFE_ROAD_FEED_UPDATE_INTERVAL` | `1800` | 30 phút |
| `SAFE_ROAD_LOG_RETENTION_DAYS` | `7` | |
| `SAFE_ROAD_DUCKDNS_DOMAIN` | `saferoad` | |
| `SAFE_ROAD_DUCKDNS_TOKEN` | `` | |
| `SAFE_ROAD_BLOCK_PAGE_IP` | `127.0.0.1` | |

---

**End of Zero-Cost SRS**

> **Commitment:** Với tài liệu này, Safe Road có thể được triển khai và vận hành với **$0 chi phí hàng tháng**, phục vụ cộng đồng bền vững mà không phụ thuộc vào ngân sách.
