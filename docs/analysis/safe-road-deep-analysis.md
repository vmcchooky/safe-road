# Safe Road — Deep Architecture & Security Analysis

> **Repository:** `github.com/vmcchooky/safe-road`  
...
Safe Road là một hệ thống chống phishing (anti-phishing) được thiết kế theo triết lý **zero-cost-first** (chi phí vận hành tối thiểu), mục tiêu triển khai trên một VPS giá rẻ khoảng **$10/tháng** (Hetzner CPX21 hoặc tương đương). Hệ thống sử dụng **Go** làm ngôn ngữ chính, với kiến trúc micro-monolith gồm 2 binary chính (`core-api`, `dns-resolver`) và 2 binary phụ (`feed-sync`, `feed-syncd`).
...
Đây là **điểm mạnh cốt lõi** của Safe Road. Triết lý fail-open được implement ở 3 lớp:
...
- Nếu Gemini API key không được set (`SAFE_ROAD_GEMINI_API_KEY` empty), AI hoàn toàn bị disable.
...
Safe Road sử dụng **3 lớp phát hiện**:
...
|----------|-------------|--------------|------|
| **Safe Road** | Lexical + Feed + AI (opt) | Trung bình | $0-$10/tháng |
...
**Kết luận:** Safe Road phù hợp cho **first-line defense** hoặc **on-premise deployment** nơi không thể gửi dữ liệu ra ngoài. Nhưng độ chính xác không đủ để thay thế các dịch vụ commercial.
...
- **Key structure**: `safe-road:analysis:<domain>` hoặc tương tự.
...
Safe Road implement **DNS over HTTPS (DoH)** endpoint tại `/dns-query` trên `dns-resolver:8081`.
...
- Có `docs/Safe_Road_OPEX_Estimate.md` và `docs/specs/opex-cost-optimization/policy.md` — rất hiếm thấy ở project cá nhân.
...
**PowerShell scripts (`scripts/safe-road.ps1`):**
...
- **Manual trigger**: Các script cần chạy thủ công. Không có cron job automation (ngoài `ops/cron/safe-road.cron.example`).
...
jobs:
  ...
  build:
    ...
    steps:
      - uses: actions/checkout@v4
      - run: docker build -t safe-road:${{ github.sha }} .
      - uses: aquasecurity/trivy-action@master
        with: { image-ref: 'safe-road:${{ github.sha }}' }
```
...
- Environment variables: `SAFE_ROAD_GEMINI_API_KEY`, `SAFE_ROAD_REDIS_ADDR`.
...
PR template của Safe Road là một **best practice đáng học hỏi**:
