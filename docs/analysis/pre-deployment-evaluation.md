...
2.  **[x] Cài đặt Tự động Offsite Backup qua `rclone`:**
    *   `scripts/safe-road.sh` và `scripts/safe-road.ps1` đã backup Redis RDB, SQLite hot backup bằng `.backup`, `.env`, Caddyfile, và upload tùy chọn qua `SAFE_ROAD_RCLONE_REMOTE`/`SAFE_ROAD_RCLONE_DEST`.
...
4.  **[x] Tối ưu hóa Trải nghiệm Trang Chặn HTTPS (HTTPS Block Page UX):**
    *   `SAFE_ROAD_DNS_BLOCK_STRATEGY` hỗ trợ `sinkhole`, `nxdomain`, `refused`, và `nullip`.
