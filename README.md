...
Legacy note: [docs/Safe_Road_SRS_Zero_Cost_v1.0.md](docs/Safe_Road_SRS_Zero_Cost_v1.0.md) is kept for historical reference only. The operator-facing production source of truth is [docs/production-completion-checklist.md](docs/production-completion-checklist.md), with deeper implementation detail kept under [docs/specs/](docs/specs/).
...
- `http://blocked.example.test/` can be sinkholed to the Safe Road block page in production.
- `https://$SAFE_ROAD_PUBLIC_HOST/block?domain=blocked.example.test` provides the canonical HTTPS explanation page.
...
The shared [ops/secrets/README.md](</D:/Go/duan/safe-road/ops/secrets/README.md>) path works for:
...
Production should set `SAFE_ROAD_ENV=production`. In that mode, `core-api` now fails startup when:

- `SAFE_ROAD_ADMIN_PASSWORD` is missing or weak
- `SAFE_ROAD_ADMIN_API_KEY` is missing or weak
...
```env
SAFE_ROAD_AGENT_ENABLED=true
SAFE_ROAD_AGENT_FEED_PRESET=production-free
SAFE_ROAD_AGENT_WEBHOOK_URL=
```
...
The baseline JSON-metrics alert rules live in [ops/alerts/safe-road-alert-rules.yaml](ops/alerts/safe-road-alert-rules.yaml), and the operator checklist is in [docs/runbooks/alert-rules.md](docs/runbooks/alert-rules.md).
...
- `feed-sync` resolves `SAFE_ROAD_AGENT_FEED_SOURCES`, then `SAFE_ROAD_AGENT_FEED_PRESET`, then `SAFE_ROAD_THREAT_FEED_SOURCE`, and runs each source once.

For Linux hosts, the equivalent shell helper supports `deploy`, `deploy-dev`, and the `SAFE_ROAD_STACK=production|dev` selector for status/log/backup helpers.
