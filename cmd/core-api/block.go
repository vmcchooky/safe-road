...
func (a *app) blockPageHandler(w http.ResponseWriter, r *http.Request) {
	...
	data := blockPageData{
		...
		Reason:          firstNonEmpty(r.URL.Query().Get("reason"), "This request was redirected because the requested domain matched a Safe Road block policy."),
		SupportEmail:    strings.TrimSpace(config.String("SAFE_ROAD_BLOCK_PAGE_SUPPORT_EMAIL", "")),
