...
echo.
echo [Safe Road] Starting local test stack...
...
echo.
echo [OK] Safe Road local stack is ready.
echo.
...
:start_prod
echo.
echo [Safe Road] Starting production stack...
"%PS_EXE%" -NoProfile -ExecutionPolicy Bypass -File "%~dp0scripts\safe-road.ps1" deploy
exit /b %ERRORLEVEL%
...
if not exist ".env" (
  if exist ".env.example" (
    ...
    echo [Safe Road] Created .env from .env.example.
  ) else (
...
"%PS_EXE%" -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "$pairs = [ordered]@{" ^
  "  'SAFE_ROAD_ENV'='local';" ^
  "  'SAFE_ROAD_CORE_API_ADDR'=':8080';" ^
  "  'SAFE_ROAD_DNS_RESOLVER_ADDR'=':8081';" ^
  "  'SAFE_ROAD_REDIS_ADDR'='redis:6379';" ^
  "  'SAFE_ROAD_ADMIN_PASSWORD'='SafeRoadLocalAdmin123!';" ^
  "  'SAFE_ROAD_ADMIN_API_KEY'='SafeRoadLocalApiKey1234567890';" ^
  "  'SAFE_ROAD_OSINT_ENABLED'='true';" ^
  "  'SAFE_ROAD_OSINT_MODE'='background_on_demand';" ^
  "  'SAFE_ROAD_OSINT_TIMEOUT_MS'='2000';" ^
  "  'SAFE_ROAD_OSINT_CACHE_TTL_SECONDS'='21600';" ^
  "  'SAFE_ROAD_OSINT_TRUSTED_DOMAINS'='gov.vn,bocongan.gov.vn,mps.gov.vn,baochinhphu.vn,thanhnien.vn,tuoitre.vn,vnexpress.net,vietnamnet.vn,nhandan.vn,vtv.vn,vov.vn';" ^
  "};" ^
...
set "URL=%~1"
set "NAME=%~2"
echo [Safe Road] Waiting for %NAME%...
"%PS_EXE%" -NoProfile -ExecutionPolicy Bypass -Command ^
