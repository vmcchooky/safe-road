...
func TestIsProductionSupportsAliases(t *testing.T) {
	t.Setenv("SAFE_ROAD_ENV", "production")
	if !IsProduction() {
		t.Fatal("expected production=true")
	}

	t.Setenv("SAFE_ROAD_ENV", "prod")
	if !IsProduction() {
...
func TestSecretStringEPrefersDirectEnv(t *testing.T) {
	t.Setenv("SAFE_ROAD_ADMIN_PASSWORD", "from-env")
	t.Setenv("SAFE_ROAD_ADMIN_PASSWORD_FILE", "")

	got, err := SecretStringE("SAFE_ROAD_ADMIN_PASSWORD")
	if err != nil {
...
func TestSecretStringEReadsFileAndTrimsWhitespace(t *testing.T) {
	...
	t.Setenv("SAFE_ROAD_ADMIN_PASSWORD_FILE", path)
	t.Setenv("SAFE_ROAD_SECRET_FILE_ROOT", dir)
...
func TestSecretStringEReturnsErrorForUnreadableFile(t *testing.T) {
	t.Setenv("SAFE_ROAD_ADMIN_PASSWORD", "")
	t.Setenv("SAFE_ROAD_ADMIN_PASSWORD_FILE", filepath.Join(t.TempDir(), "missing.txt"))
	t.Setenv("SAFE_ROAD_SECRET_FILE_ROOT", t.TempDir())

	if _, err := SecretStringE("SAFE_ROAD_ADMIN_PASSWORD"); err == nil {
