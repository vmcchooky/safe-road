...
if not "%ERRORLEVEL%"=="0" (
  echo [Safe Road] Docker is not running; local containers are already stopped.
...
:stop_dev
echo.
echo [Safe Road] Stopping local test stack...
docker compose -f docker-compose.yml -f docker-compose.dev.yml --profile feed-sync down
set "CODE=%ERRORLEVEL%"
call :cleanup_sqlite_sidecars
if not "%CODE%"=="0" exit /b %CODE%
echo.
echo [OK] Safe Road local stack stopped.
exit /b 0

:stop_prod
echo.
echo [Safe Road] Stopping production stack...
...
if exist "data\safe-road.db-shm" del /f /q "data\safe-road.db-shm" >nul 2>nul
if exist "data\safe-road.db-wal" del /f /q "data\safe-road.db-wal" >nul 2>nul
exit /b 0
