...
cert_src="${1:-${SAFE_ROAD_DOT_CERT_SOURCE:-}}"
key_src="${2:-${SAFE_ROAD_DOT_KEY_SOURCE:-}}"
target_dir="${SAFE_ROAD_DOT_CERT_TARGET_DIR:-${project_dir}/ops/certs/dot}"

if [ -z "$cert_src" ] || [ -z "$key_src" ]; then
  echo "usage: scripts/export-dot-cert.sh /path/to/fullchain.pem /path/to/privkey.pem" >&2
  echo "or set SAFE_ROAD_DOT_CERT_SOURCE and SAFE_ROAD_DOT_KEY_SOURCE." >&2
  exit 2
fi
